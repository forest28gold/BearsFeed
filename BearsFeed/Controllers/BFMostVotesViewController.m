...
@interface BFMostVotesViewController () <UITableViewDragLoadDelegate, UIGestureRecognizerDelegate, UIDocumentInteractionControllerDelegate>
{
    int _dataCount;
    BOOL tempIsOn;
}
...
@implementation BFMostVotesViewController
...
- (void)viewDidLoad {
    ...
    SVPROGRESSHUD_SHOW;
    [self initLoadMostVotesData];
}

- (void)viewWillAppear:(BOOL)animated {
    [super viewWillAppear:animated];
    
    [GlobalData sharedGlobalData].g_strSelectedTab = SELECT_MOST_VOTES;
...
- (void)initLoadMostVotesData {
    
    [GlobalData sharedGlobalData].g_arrayMostVotesPost = [[NSMutableArray alloc] init];
    
    _dataCount += LOAD_DATA_COUNT;
    
    QueryOptions *queryOption = [QueryOptions new];
    [queryOption addRelated:@"postUser"];
    queryOption.sortBy = @[@"likeCount DESC"];
        ...
        if (posts.data.count > 0) {
            ...
            for (Post *post in posts.data) {
                ...
                if (![record.reportTypeArray containsObject:[GlobalData sharedGlobalData].g_userData.userId]) {
                    [[GlobalData sharedGlobalData].g_arrayMostVotesPost addObject:record];
                }
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    ...
    return [GlobalData sharedGlobalData].g_arrayMostVotesPost.count;
}

- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    
    PostData *postData = [GlobalData sharedGlobalData].g_arrayMostVotesPost[indexPath.row];
    ...
    if ([postData.type isEqualToString:POST_TYPE_PHOTO]) {
        ...
        [cell.btnUpVote addTarget:self action:@selector(onMostVotesUpVote:) forControlEvents:UIControlEventTouchUpInside];
        [cell.btnDownVote addTarget:self action:@selector(onMostVotesDownVote:) forControlEvents:UIControlEventTouchUpInside];
        [cell.btnReadMore addTarget:self action:@selector(onMostVotesReadMore:) forControlEvents:UIControlEventTouchUpInside];
        [cell.btnComment addTarget:self action:@selector(onMostVotesComment:) forControlEvents:UIControlEventTouchUpInside];
        [cell.btnReply addTarget:self action:@selector(onMostVotesReply:) forControlEvents:UIControlEventTouchUpInside];
        [cell.btnPhoto addTarget:self action:@selector(onMostVotesDetailPhotoView:) forControlEvents:UIControlEventTouchUpInside];
        [cell.btnShareFB addTarget:self action:@selector(onShareMostVotesPostToFacebook:) forControlEvents:UIControlEventTouchUpInside];
        [cell.btnShareTW addTarget:self action:@selector(onShareMostVotesPostToTwitter:) forControlEvents:UIControlEventTouchUpInside];
        [cell.btnShareINS addTarget:self action:@selector(onShareMostVotesPostToInstagram:) forControlEvents:UIControlEventTouchUpInside];
        
        cell.imgAvatar.userInteractionEnabled = YES;
        UITapGestureRecognizer *tapAvatar = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(onMostVotesDetailUserProfile:)];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [GlobalData sharedGlobalData].g_postData = [GlobalData sharedGlobalData].g_arrayMostVotesPost[indexPath.row];
...
-(void)onMostVotesUpVote:(id)sender {
    ...
    [BFUtility likePostInBackground:indexPath dataArray:[GlobalData sharedGlobalData].g_arrayMostVotesPost];
    
    [m_tableView reloadData];
}

-(void)onMostVotesDownVote:(id)sender {
    
    UIButton *btn = (UIButton*)sender;
    CGRect buttonFrameInTableView = [btn convertRect:btn.bounds toView:self.m_tableView];
    NSIndexPath *indexPath = [self.m_tableView indexPathForRowAtPoint:buttonFrameInTableView.origin];
    
    [BFUtility dislikePostInBackground:sender indexPath:indexPath dataArray:[GlobalData sharedGlobalData].g_arrayMostVotesPost];
    
    [m_tableView reloadData];
}

-(void)onMostVotesReadMore:(id)sender {
    ...
    [[GlobalData sharedGlobalData].g_arrayMostVotesPost replaceObjectAtIndex:indexPath.row withObject:postData];
    [m_tableView reloadData];
}

-(void)onMostVotesDetailPhotoView:(id)sender {
...
-(void)onMostVotesDetailUserProfile:(UITapGestureRecognizer*)sender {
...
-(void)onMostVotesComment:(id)sender {
...
-(void)onMostVotesReply:(id)sender {
...
-(void)onShareMostVotesPostToFacebook:(id)sender {
    
    UIButton *btn = (UIButton*)sender;
    CGRect buttonFrameInTableView = [btn convertRect:btn.bounds toView:self.m_tableView];
    NSIndexPath *indexPath = [self.m_tableView indexPathForRowAtPoint:buttonFrameInTableView.origin];
    
    PostData *record = [GlobalData sharedGlobalData].g_arrayMostVotesPost[indexPath.row];
    
    [BFUtility shareToSicial:0 viewCtrl:self text:record.content imageUrl:record.filePath];
}

-(void)onShareMostVotesPostToTwitter:(id)sender {
...
-(void)onShareMostVotesPostToInstagram:(id)sender {
